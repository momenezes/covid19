...
numero_fp <- scales::label_comma(accuracy = .01, big.mark = ".", decimal.mark = ",")
```
...
filter(jhucovid19, Country %in% top50) %>% 
  ...
  summarise(TotCasos = last(ConfirmedCases), 
            ...
            `Letal.%` = numero_fp(round(TotMortes/TotCasos*100,2)), 
            Recup = last(Recovered), 
            `%NovosCa` = numero_fp(round((last(ProgRateCases) -1)*100,2)), 
            `%NovasMo` = numero_fp(round((last(ProgRateDeaths) -1)*100,2)), 
...
tabgrafWorld <- select(tabelaWorld,Pais=Country, Casos = TotCasos, #`Evol.C` = ConfirmedCasesL,
                       `Novos C` = NewCasesT,  
                       `Novos C (%)`=`%NovosCa`, `Evol.NC` = NewCasesL, 
                       Mortes = TotMortes, #`Evol.M` = DeathsL, 
                       `Novas M` = NewDeathsT, 
                       `Novas M (%)` = `%NovasMo` , `Evol.NM` = NewDeathsL, 
                       `Letal.(%)` = `Letal.%`)
reactable(tabgrafWorld,
          ...
          columns = list(Casos = colDef(format = colFormat(separators = TRUE, locales = "pt-BR")),
                         ...
                         `Novos C (%)` = colDef(align = "right"),
                         `Novas M (%)` = colDef(align = "right"),
                         `Letal.(%)` = colDef(align = "right"),
                         ...
                         `Evol.NC` = colDef(align = "center", cell= function(value, index) {
                           sparkline(tabgrafWorld$`Evol.NC`[[index]])
                         }),
                         `Evol.NM` = colDef(align = "center", cell= function(value, index) {
...
```
```{r}
kable(briototais.df, format.args = list(big.mark = ".", decimal.mark = ","), align = "r")
```
...
casosfull_brio_gr %>% 
  ...
  summarise(Casos = sum(last_available_confirmed),
            ...
            `Letal. (%)` = numero_fp(round(Mortes/Casos*100,1)),
            `Casos/1Mi` = round(Casos/last(População)*1e6,0), 
            `Mortes/1Mi` = round(Mortes/last(População)*1e6,0),
            `Novos C` = last(NovCasos),
            `Novos C (%)` = numero_fp(round(last(ProgRateCases)*100,2)), 
            `Novas M` = last(NovMortes),
            `Novas M (%)` = numero_fp(round(last(ProgRateDeaths)*100,2)), 
            ) -> tabelaBR
...
reactable(tabelaBR, defaultPageSize = 27, bordered = TRUE, striped = TRUE, 
          ...
          columns = list(Casos = colDef(format = colFormat(separators = TRUE, locales = "pt-BR")),
                         Mortes = colDef(format = colFormat(separators = TRUE, locales = "pt-BR")),
                         `Letal. (%)`= colDef(align = "right"),
                         `Casos/1Mi` = colDef(format = colFormat(separators = TRUE, locales = "pt-BR")),
                         `Mortes/1Mi` = colDef(format = colFormat(separators = TRUE, locales = "pt-BR")),
                         `Novos C` = colDef(format = colFormat(separators = TRUE, locales = "pt-BR")),
                         `Novos C (%)` = colDef(align = "right"),
                         `Novas M` = colDef(format = colFormat(separators = TRUE, locales = "pt-BR")),
                         `Novas M (%)` = colDef(align = "right")),
