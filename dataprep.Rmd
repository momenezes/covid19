...
Filtrei apenas os anos de 2019 e 2020 (previsão).

```{r eval = FALSE}
pop1920 <- populacao %>% select(-LocID, -VarID, -Variant, -MidPeriod) %>% filter(Time == 2019 | Time == 2020) %>%
  group_by(Location, Time) %>% summarise(Year = max(Time), PopMale = last(PopMale), PopFemale = last(PopFemale), PopTotal = last(PopTotal), PopDensity = last(PopDensity)) %>% select(-Time) %>% dplyr::ungroup()
```
