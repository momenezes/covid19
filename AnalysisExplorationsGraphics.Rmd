...
```{r country}
covid19 <- rename(covid19, Country = `countriesAndTerritories`, DateRep = dateRep, Cases = cases, Deaths = deaths, Day = day, Month = month) %>% select(Country, DateRep, Cases, Deaths, popData2018)
```
...
#### Total de Casos pelos dados da EU Open Data Portal: `r format(ultimadata - 1, '%d-%m-%Y')`
...
```{r}
Italia %>% select(-Country, -popData2018) %>% top_n(20, DateRep) %>% kable()
```

O mesmo para o Brasil, últimos 20 dias.
```{r}
Brasil %>% select(-Country, -popData2018) %>% top_n(20, DateRep) %>% kable()
```
...
```{r}
bind_rows(Italia14,Brasil13) %>% ggplot() + geom_point(aes(x = Dia, y = CumCases, color=Country), stat = "identity", position = "dodge") + labs(title = "Número cumulativo de casos", subtitle = paste("CumCases>=14 & CumCases<=", LimiteCorte), caption = Fonte, color = "") + scale_color_brewer(palette = "Set1")  #-> g
#fig <- ggplotly(g)
#fig
```
...
```
### Total de Casos pelos dados da John Hopkins University (JHU): `r format(ultimadata - 1, '%d-%m-%Y')`
...
```{r}
filter(jhucovid19, Country == "France") %>% select(-Country) %>% top_n(10,DateRep) %>% kable(caption = "França")
```


```{r}
filter(jhucovid19, Country == "Italy") %>% select(-Country) %>% top_n(10,DateRep) %>% kable(caption = "Itália")
```

```{r}
filter(jhucovid19, Country == "Spain") %>% select(-Country) %>% top_n(10,DateRep) %>% kable(caption = "Espanha")
```

```{r}
filter(jhucovid19, Country == "Brazil") %>% select(-Country) %>% top_n(10,DateRep) %>% kable(caption = "Brasil")
```
...
### Lista de países com mais de 500 casos, ordenados por número de casos confirmados: `r format(ymd(hoje) - 1, '%d-%m-%Y')`
...
bind_rows(Italia14,Brasil13) %>% 
  ...
  guides(shape = FALSE) + scale_color_brewer(palette = "Set1") # -> g
#fig <- ggplotly(g)
#fig
```
...
filter(jhucovid19, Country %in% c("Italy", "Brazil", "France", "Spain", "Germany", "USA")) %>% 
  ...
  top_n(-35, Dia) %>% 
  ggplot(aes(x = Dia, y = ConfirmedCases)) + geom_point(aes(shape = Country, color = Country)) + 
  labs(y = "Número de Casos", x = "Núm Dias após o 14º caso", shape = "", color= "", title = "Evolução dos 35 primeiros dias após o 14º caso", caption = Fonte)  + scale_color_brewer(palette = "Set1")  # -> g
#fig <- ggplotly(g)
#fig
```
...
filter(jhucovid19, Country %in% c("Italy", "Brazil", "France", "USA", "Japan")) %>% 
  ...
  labs(y = "Número de Casos", x = "Núm Dias após o 30º caso", shape = "", color= "", 
       title = "Evolução após o 30º caso", caption = Fonte)  + scale_color_brewer(palette = "Set1")  # -> g
#fig <- ggplotly(g)
#fig
```

```{r tidy.opts=list(options(knitr.kable.NA = ''))}
filter(jhucovid19, Country %in% c("Italy", "Brazil", "France", "Spain", "Germany", "USA", "Chile")) %>% 
  filter(ConfirmedCases >= 30 ) %>% 
