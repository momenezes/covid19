...
p <- dfjhu %>% group_by(Country) %>% arrange(desc(ConfirmedCases)) %>% summarise(ConfirmedCases = max(ConfirmedCases)) %>%
  ...
  geom_bar(aes(y = ConfirmedCases/1000),fill = "orange", stat = "identity", position = "dodge") + 
...
brestados %>% mutate(Letalidade = paste0(round(Letalidade*100,1),"%")) %>%
  datatable(rownames = F, autoHideNavigation = T, caption = "Detalhamento Brasil por Estados") 
```
...
### Total de Casos pelos dados do Brasil.io: `r format(ultimadata, '%d-%m-%Y')` {#BRtotKagg}
...
casosfull_brio %>% 
  ...
  rename(Estado = state) %>%
  datatable(rownames = F, autoHideNavigation = T, caption = "Detalhamento Brasil por Estados (Brasil.io)") 
```
...
#  summarise(Confirmados = sum(last_available_confirmed),Mortes = sum(last_available_deaths)) %>% 
#  rename(Estado = state) %>%
#  group_by(Country) %>% arrange(desc(ConfirmedCases)) %>% summarise(ConfirmedCases = max(ConfirmedCases)) %>%
# title = paste0("Casos Confirmados em ", format(today(), "%d-%m-%Y"))
p <- casosfull_brio %>% 
  filter(place_type == "state") %>% 
  group_by(state,date) %>% 
  filter(is_last == "TRUE") %>%
...
q <- casosfull_brio %>% 
  ...
  filter(is_last == "TRUE") %>%
  summarise(Casos = sum(last_available_confirmed), População = last(estimated_population_2019), Mortes = sum(last_available_deaths)) %>%
  ggplot(aes(x = reorder(state,Mortes), y = Mortes)) + geom_col() + 
  geom_bar(fill = "#666666", stat = "identity", position = "dodge") + 
  #geom_bar_text(position = "dodge", min.size = 2 ) +
  geom_text(aes(x = state, y = Mortes, label = Mortes), hjust = -0.5, size = 3, color = "darkblue") +
  geom_text(aes(x = state, y = 0, label = state), hjust = 1.5, size = 3.5, color = "darkblue") +
  labs(x = "", y = "", title = paste0("Mortes em ", format(ultimadata, "%d-%m-%Y"))) +
  scale_y_continuous(breaks = NULL, labels = NULL, expand = expansion(mult = 0.15)) + scale_x_discrete(breaks = NULL) +
  labs_pubr() + 
  theme(plot.background = element_rect(fill = "#DDDDDD"), 
        axis.text.x = element_text(color = "#111111"),
        axis.text.y = element_text(color = "#111111"),
