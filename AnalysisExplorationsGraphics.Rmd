...
```{r}
covid19 %>% 
  summarise(`Casos no Mundo (EU Open Data Portal)` = sum(Cases)) %>% 
  kable()
```
...
Italia %>% select(-Country, -Continent, -popData2018) %>% top_n(20, DateRep) %>% 
  mutate(DateRep = format(DateRep, "%d-%m-%Y")) %>% datatable(rownames = F, autoHideNavigation = T )
...
filter(jhucovid19, Country == "France") %>% select(-Country, -Continent) %>% top_n(10,DateRep) %>% 
   mutate(DateRep = format(DateRep, "%d-%m-%Y")) %>% 
  datatable(rownames = F, autoHideNavigation = T, caption = "França" )
...
filter(jhucovid19, Country == "Italy") %>% select(-Country, -Continent) %>% top_n(10,DateRep) %>% 
   mutate(DateRep = format(DateRep, "%d-%m-%Y")) %>%
  datatable(rownames = F, autoHideNavigation = T ,caption = "Itália")
```

```{r}
filter(jhucovid19, Country == "Spain") %>% select(-Country, -Continent) %>% top_n(10,DateRep) %>% 
   mutate(DateRep = format(DateRep, "%d-%m-%Y")) %>%
  datatable(rownames = F, autoHideNavigation = T , caption = "Espanha")
```

```{r}
filter(jhucovid19, Country == "Brazil") %>% select(-Country, -Continent) %>% top_n(10,DateRep) %>% 
   mutate(DateRep = format(DateRep, "%d-%m-%Y")) %>%
  datatable(rownames = F, autoHideNavigation = T , caption = "Brasil")
```
...
filter(jhucovid19, Country %in% c("Italy", "Brazil", "France", "Spain", "Germany", "US", "Chile")) %>% 
  ...
  pivot_wider(id_cols =  DateRep, names_from = Country, values_from = ConfCases) %>% 
   mutate(DateRep = format(DateRep, "%d-%m-%Y")) %>%
  datatable(rownames = F, autoHideNavigation = T, caption = "Evolução dos casos após o 30º caso confirmado" )
...
txprogtops %>% 
  ...
  select(Country,NewCasesMA,ConfirmedCases,Dia) %>%
    ...
    geom_line(aes(color = Country), size = 1.5) + 
    labs(x = "Casos Confirmados", y = "Novos Casos", title = "Casos Novos x Confirmados", subtitle = glue::glue("Média Móvel (n=",nma,")"), color ="", caption = Fonte)  +
    scale_color_discrete(l = 40,c = 150)  + theme(legend.position = "top") -> g1
g1
```
...
txprogtops %>%
  ...
  guides(shape = "none", linetype="none") + 
  scale_color_brewer(palette = "Set1") + theme(legend.position = "top") -> g2
g2
```
...
jhucovid19 %>% 
  ...
  annotate("text", x = 40, y = 10**3.7, label = "Dobra 5 dias") +
...
txprogpar %>%  
  ...
  select(Country,DateRep,ProgRateCases) %>%
  pivot_wider(names_from = Country, values_from = ProgRateCases) %>% 
  datatable(rownames = F, autoHideNavigation = T, caption = "Taxa de Progressão") 
```
