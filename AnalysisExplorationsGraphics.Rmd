...
```{r}
jhucovid <- read_csv("https://data.humdata.org/hxlproxy/api/data-preview.csv?url=https%3A%2F%2Fraw.githubusercontent.com%2FCSSEGISandData%2FCOVID-19%2Fmaster%2Fcsse_covid_19_data%2Fcsse_covid_19_time_series%2Ftime_series_19-covid-Confirmed.csv&filename=time_series_2019-ncov-Confirmed.csv")
```

```{r}
jhucovid19_confirmed <- gather(jhucovid, key = "DateRep", value = "ConfirmedCases", 5:60)
```

```{r}
filter(jhucovid19_confirmed, `Country/Region` == "Brazil")
```

```{r}
jhucovid_D <- read_csv("https://data.humdata.org/hxlproxy/api/data-preview.csv?url=https%3A%2F%2Fraw.githubusercontent.com%2FCSSEGISandData%2FCOVID-19%2Fmaster%2Fcsse_covid_19_data%2Fcsse_covid_19_time_series%2Ftime_series_19-covid-Deaths.csv&filename=time_series_2019-ncov-Deaths.csv")
```

```{r}
jhucovid19_deaths <- gather(jhucovid_D, key = "DateRep", value = "Deaths", 5:60)
```

```{r}
jhucovid_R <- read_csv("https://data.humdata.org/hxlproxy/api/data-preview.csv?url=https%3A%2F%2Fraw.githubusercontent.com%2FCSSEGISandData%2FCOVID-19%2Fmaster%2Fcsse_covid_19_data%2Fcsse_covid_19_time_series%2Ftime_series_19-covid-Recovered.csv&filename=time_series_2019-ncov-Recovered.csv")
```

```{r}
jhucovid_recov <- gather(jhucovid_R, key = "DateRep", value = "Recovered", 5:60)
```
...
```{r}
filter(jhucovid19, `Country/Region` == "China")
```

```{r}
length(unique(jhucovid19$`Country/Region`))
```

Alguns acertos e transformações

```{r}
jhucovid19 <- rename(jhucovid19, Country = `Country/Region`)
```

```{r}
filter(jhucovid19, Country == "China")
```
...
jhucovid19 <- arrange(jhucovid19, Country) %>% arrange(DateRep)
...
```{r}
jhucovid19_semChina <- jhucovid19 %>% filter(Country != "China")
```
```{r}
China <- jhucovid19 %>% filter(Country == "China")
China <- group_by(China, `Province/State`) %>% 
  mutate(NewDeaths = Deaths - lag(Deaths), 
         #NewDeaths = if_else(is.na(NewDeaths), if_else(Deaths > 0, Deaths, 0), NewDeaths),
         NewCases = ConfirmedCases - lag(ConfirmedCases) + lag(NewDeaths), 
         NewCases = if_else(is.na(NewCases), if_else(ConfirmedCases > 0, ConfirmedCases, 0), NewCases))
```

```{r}
jhucovid19 <- bind_rows(jhucovid19_semChina, China) %>% arrange(Country, `Province/State`)
```
...
```{r}
filter(jhucovid19, Country == "China") #%>% summarise(sum(ConfirmedCases))
```


```{r}
jhucovid19 %>% group_by(Country) %>% summarise(Confirmed = sum(NewCases) - sum(NewDeaths, na.rm=T)) %>% arrange(desc(Confirmed))
```
