...
```{r}
Italia14 <- Italia %>% 
  filter(CumCases >= 14 & CumCases <= 700) %>% 
  select(Country,DateRep, Cases,Deaths,CumCases)
Brasil13 <- Brasil %>% 
  filter(CumCases >= 13 & CumCases <= 700) %>% 
  select(Country,DateRep,Cases,Deaths,CumCases)
```
```{r}
...
```{r}
bind_rows(Italia14,Brasil13) %>% ggplot() + geom_point(aes(x = Dia, y = CumCases, color=Country), stat = "identity", position = "dodge")  
```
...
filter(jhucovid19, Country %in% c("Italy", "Brazil", "France", "Spain", "Germany")) %>% 
  filter(ConfirmedCases >= 1 ) %>% 
  select(Country,DateRep,ConfirmedCases,Deaths) %>% 
  pivot_wider(names_from = Country, values_from = ConfirmedCases)
```
