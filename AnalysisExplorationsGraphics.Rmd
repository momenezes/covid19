...
bind_rows(select(Italia,Country, DateRep, CumCases, Cases, Deaths), select(Brasil, Country, DateRep, CumCases, Cases, Deaths)) %>% filter(DateRep >= "2020-02-22") %>%
  ...
  labs(y = "Núm de Casos Reportados", x = "Data", title = "Evolução dos Casos na Italia e Brasil, a partir de 22/02/2020", fill = "") + scale_fill_brewer(palette = "Set1") -> g
fig <- ggplotly(g)
fig
```
...
bind_rows(sochina,restomundo) %>% filter(DateRep >= "2020-02-01") %>%
  ...
  labs(x = "Data", title = "Número de Novos Casos China x Resto do Mundo, a partir de 01/02/2020",fill="") + 
  scale_fill_brewer(palette = "Set1") -> g
fig <- ggplotly(g)
fig
```
...
filter(jhucovid19, Country %in% c("China","Italy","Iran","Germany","Japan"), DateRep >= "2020-02-01") %>% 
  ...
  labs(x = "Data Reportado", y = "Novos Casos", title = "Novos Casos a partir de 01 de Fev 2020", fill="") -> g
fig <- ggplotly(g)
fig
```
...
bind_rows(Italia14,Brasil13) %>% 
  ...
  labs(y = "Número de Casos", x = "Núm Dias após o 1º caso", color = "", title = "Evolução dos 25 primeiros dias após o 1º caso") + guides(shape = FALSE) -> g
fig <- ggplotly(g)
fig
```
...
filter(jhucovid19, Country %in% c("Italy", "Brazil", "France", "Spain", "Germany", "USA")) %>% 
  ...
  labs(y = "Número de Casos", x = "Núm Dias após o 1º caso", shape = "", color= "", title = "Evolução dos 55 primeiros dias após o 1º caso")  + scale_color_brewer(palette = "Set1") -> g
fig <- ggplotly(g)
fig
```
...
filter(jhucovid19, Country %in% c("Italy", "Brazil", "France", "Spain", "Germany", "USA")) %>% 
  ...
  top_n(-35, Dia) %>% 
  ggplot(aes(x = Dia, y = ConfirmedCases)) + geom_point(aes(shape = Country, color = Country)) + 
  labs(y = "Número de Casos", x = "Núm Dias após o 14º caso", shape = "", color= "", title = "Evolução dos 35 primeiros dias após o 14º caso")  + scale_color_brewer(palette = "Set1") -> g
fig <- ggplotly(g)
fig
```
...
filter(jhucovid19, Country %in% c("Italy", "Brazil", "France", "USA", "Japan")) %>% 
  ...
  mutate(Dia = row_number()) %>%
  ggplot(aes(x = Dia, y = ConfirmedCases)) + geom_point(aes(shape = Country, color = Country)) + 
  labs(y = "Número de Casos", x = "Núm Dias após o 30º caso", shape = "", color= "", title = "Evolução após o 30º caso")  + scale_color_brewer(palette = "Set1") -> g
fig <- ggplotly(g)
fig
```
...
```{r tidy.opts=list(options(knitr.kable.NA = ''))}
filter(jhucovid19, Country %in% c("Italy", "Brazil", "France", "Spain", "Germany", "Japan","USA")) %>% 
  group_by(Country) %>% 
  filter(ConfirmedCases >= 30) %>% 
  arrange(DateRep) %>%
  ...
  ungroup() %>%
  select(Country,DateRep,ConfirmedCases,ProgRateCases,Deaths,ProgRateDeaths) %>%
  arrange(Country, DateRep) %>%
  pivot_wider(id_cols =  DateRep, names_from = Country, values_from = ProgRateCases) %>%
  arrange(DateRep) %>%
  kable(caption="Taxa de Progressão de Casos Confirmados (após 30º caso)")
```
