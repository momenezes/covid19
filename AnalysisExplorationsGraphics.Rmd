...
#hoje <- "2020-03-17"
arqdest <- paste("COVID-19-geographic-distribution-worldwide-",hoje,".xls",sep="")
```
...
if (!file.exists(paste("~/datasets/ECDC_Europe",arqdest,sep="/")) || !file.exists(paste("./data",arqdest,sep="/"))) {
  curl::curl_download(paste(covid19URLbase,hoje,".xls",sep=""), paste("~/datasets/ECDC_Europe",arqdest,sep="/"))
...
jhutscovid19_confirmed <- read_csv("https://data.humdata.org/hxlproxy/api/data-preview.csv?url=https%3A%2F%2Fraw.githubusercontent.com%2FCSSEGISandData%2FCOVID-19%2Fmaster%2Fcsse_covid_19_data%2Fcsse_covid_19_time_series%2Ftime_series_19-covid-Confirmed.csv&filename=time_series_2019-ncov-Confirmed.csv")
...
```{r}
filter(jhucovid19_confirmed, `Country/Region` == "Brazil") %>% tail()
```
...
```{r}
filter(jhucovid19, Country == "China" || Country == "Italy" || Country == "Iran" || Country == "Germany" || Country == "France", DateRep >= "2020-02-01") %>% 
