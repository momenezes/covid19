...
```{r}
covid19 %>% 
  summarise(Casos = sum(Cases, na.rm = T), 
            Mortes = sum(Deaths, na.rm = T), 
            Letalidade = sprintf("%s%%", round(Mortes/Casos*100,2))) %>%
  kable(align = "r")
```
...
covid19 %>% 
  group_by(Continent) %>% 
  summarise(Casos = as.integer(sum(Cases, na.rm = T)), 
            Mortes = as.integer(sum(Deaths, na.rm = T)), 
...
```{r}
Italia %>% select(-Country, -Continent, -popData) %>% top_n(20, DateRep) %>% 
...
```{r}
Brasil %>% select(-Country, -Continent, -popData) %>% top_n(20, DateRep) %>% 
...
filter(covid19, DateRep > "2020-02-01") %>% 
  ...
  summarise(numCasos = sum(Cases, na.rm = T)) %>% 
...
```{r}
restomundo <- group_by(restomundo, DateRep) %>% mutate(Cases = sum(Cases, na.rm = T)) %>% ungroup()
```
...
covid19 %>% 
  group_by(Country) %>% 
  summarise(Casos = sum(Cases, na.rm = T), Mortes = sum(Deaths, na.rm = T)) %>% 
  arrange(desc(Casos)) %>% 
...
numNAsCont <- CasosRemovidos %>% count() %>% as.numeric()
numCasosSemCont <- CasosRemovidos %>% group_by(Country) %>% summarise(Casos = sum(ConfirmedCases, na.rm = T)) %>% summarise(CasosTot = sum(Casos, na.rm = T)) %>% select(CasosTot) %>% as.numeric()
```
...
```{r}
filter(jhucovid19, Country == "France") %>% select(-Country, -Continent, -Code) %>% top_n(10,DateRep) %>% 
   mutate(DateRep = format(DateRep, "%d-%m-%Y")) %>% 
...
```{r}
filter(jhucovid19, Country == "Italy") %>% select(-Country, -Continent, -Code) %>% top_n(10,DateRep) %>% 
   mutate(DateRep = format(DateRep, "%d-%m-%Y")) %>%
  datatable(rownames = F, autoHideNavigation = T ,caption = "Itália", options = list(paging = FALSE, searching = FALSE))
```

```{r}
filter(jhucovid19, Country == "Spain") %>% select(-Country, -Continent, -Code) %>% top_n(10,DateRep) %>% 
   mutate(DateRep = format(DateRep, "%d-%m-%Y")) %>%
  datatable(rownames = F, autoHideNavigation = T , caption = "Espanha", options = list(paging = FALSE, searching = FALSE))
```

```{r}
filter(jhucovid19, Country == "Brazil") %>% select(-Country, -Continent, -Code) %>% top_n(10,DateRep) %>% 
   mutate(DateRep = format(DateRep, "%d-%m-%Y")) %>%
...
**Após diversas mudanças na página do Worldometer, estou desistindo de manter estes _web scraping_ **
...
womcovid19 <- womcovid19 %>% 
  ...
  mutate_at(vars(TotalCases,NewCases,TotalDeaths,NewDeaths,TotalRecovered,ActiveCases,SeriousCritical,Cases1M, Deaths1M), str_remove_all ,"," )  %>%
