...
O gráfico abaixo é interessante para ver que os comportamentos são muito parecidos. Usei uma média móvel de 3 observações.

```{r mediamovel}
setDT(txprogtops)     # converts test to a data.table in place
setkey(txprogtops,DateRep,Country)
txprogtops[,NewCasesMA:=as.numeric(frollmean(NewCases,n=3, align='right',hasNA=TRUE)), by=Country]
...
txprogtops %>% 
  ...
  select(Country,NewCasesMA,ConfirmedCases,Dia) %>%
    ...
    labs(x = "Casos Confirmados", y = "Novos Casos", title = "Média Móvel (n=3)", color ="")  + 
