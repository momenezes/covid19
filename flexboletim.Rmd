...
#  group_by(Estado, date) %>%
  mutate(ConfirmedCases = sum(last_available_confirmed), Deaths = sum(last_available_deaths), ConfCasesL = list(new_confirmed)) %>% ungroup() %>% ungroup()
```
...
```{r}
casosfull_brio %>% filter(place_type == "state") %>% 
  group_by(Estado) %>% arrange(date) %>%
  summarise(NovosCasosL = list(new_confirmed), NovasMortesL = list(new_deaths)) -> casosbr_estado
```

```{r}
tabelaBR <- left_join(tabelaBR, casosbr_estado, by = "Estado")
```
...
reactable(tabelaBR, defaultPageSize = 27, bordered = TRUE, striped = TRUE, 
          ...
          columns = list(Casos = colDef(format = colFormat(separators = TRUE, locales = "pt-BR")),
                         ...
                         `Letalidade %`= colDef(format = colFormat(separators = TRUE, locales = "pt-BR")),
                         ...
                         `Novos Casos` = colDef(format = colFormat(separators = TRUE, locales = "pt-BR")),
                         `Novos Casos %` = colDef(format = colFormat(separators = TRUE, locales = "pt-BR")),
                         `Novas Mortes` = colDef(format = colFormat(separators = TRUE, locales = "pt-BR")),
                         `Novas Mortes %` = colDef(format = colFormat(separators = TRUE, locales = "pt-BR")),
                         NovosCasosL = colDef(align = "center", cell= function(value, index) {
                           sparkline(tabelaBR$NovosCasosL[[index]])
                         }),
                         NovasMortesL = colDef(align = "center", cell= function(value, index) {
                           sparkline(tabelaBR$NovasMortesL[[index]])
                         })
                         ),
...
filter(jhucovid19, Country %in% top20) %>% 
  ...
  reactable(defaultPageSize = 20, bordered = TRUE, striped = TRUE, 
          ...
          columns = list(Casos = colDef(format = colFormat(separators = TRUE, locales = "pt-BR")),
                         Mortes = colDef(format = colFormat(separators = TRUE, locales = "pt-BR")),
                         `Letalidade %` = colDef(format = colFormat(separators = TRUE, locales = "pt-BR")),
                         #Recuperados = colDef(format = colFormat(separators = TRUE, locales = "pt-BR")),
                         `NovosCasos %` = colDef(format = colFormat(separators = TRUE, locales = "pt-BR")),
                         `NovasMortes %` = colDef(format = colFormat(separators = TRUE, locales = "pt-BR")),
                         Casos1M = colDef(format = colFormat(separators = TRUE, locales = "pt-BR")),
                         Mortes1M = colDef(format = colFormat(separators = TRUE, locales = "pt-BR")),
                         NDias1Caso = colDef(align = "right", format = colFormat(separators = TRUE, locales = "pt-BR"))
                         ),
          defaultSorted = list(Casos = "desc"))
