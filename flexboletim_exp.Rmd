...
```{r eval = FALSE}
totestados <- totestados %>% mutate(Estado = factor(Estado, ordered=T)) 
totestados$Estado <- reorder(totestados$Estado, -totestados$Casos)
```
...
### Casos por Estado {data-width=700}

```{r }
# nao consigo fazer o grafico com os Estados ordenados pelo numero de Casos
highchart() %>% 
  # Data
  hc_add_series(totestados, "bar", hcaes(Estado,Casos), name = "Casos") %>%
  ...
  hc_xAxis( type = 'category')  -> hcCasos

highchart() %>% 
  # Data
  hc_add_series(totestados, "bar", hcaes(x = Estado, y = Mortes),name="Mortes") %>%
  # Optiosn for each type of series
  hc_plotOptions(
    series = list(
      showInLegend = FALSE,
  ...
  # Axis
  hc_yAxis(
    title = list(text = "Mortes"),
    labels = list(format = "{value}")
  ) %>% 
  hc_xAxis( type = 'category') -> hcMortes

hw_grid(hcCasos, hcMortes)
...
## Row 4 {data-height=500, .no-mobile}
...
```{r}
totestados %>%
  e_charts(x = Estado, elementId = "casosM") %>%
  e_bar(Casos, legend = FALSE, name = "Casos") %>% 
  e_labels(position = "right") %>% 
  e_tooltip(
    trigger = "item",
...
selshd <- SharedData$new(selecionados, ~Country, group = "Selecionados (Shift+Click Vários)")
yax <- list(type = "log", 
           range = yrange,
           zeroline = FALSE,
           showline = FALSE, 
...
fig <- layout(fig, dragmode = "zoom", height = 480, width = 500, yaxis = yax, xaxis = xax, responsive = TRUE)  %>%
 highlight(on = "plotly_click", 
            persistent = FALSE,  
            opacityDim = getOption("opacityDim", 0.05), 
            off="plotly_doubleclick",
